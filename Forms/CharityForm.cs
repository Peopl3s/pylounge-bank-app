...
namespace BankApp.Forms
{
    public partial class CharityForm : Form
    {
        ...
        private void CharityForm_Load(object sender, EventArgs e)
        {
            ...
            var operators = BankService.GetBankServiceByType("Charity");
            CharityServiceComboBox.DataSource = operators;
            CharityServiceComboBox.ValueMember = "id_service";
            CharityServiceComboBox.DisplayMember = "service_name";
        }

        private void CharityPaymentButton_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Пожалуйста, введите корректный номер лицевого счёта", "Отмена", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            if (error == false)
            {
                ...
                {
                    BankService.MakeTransactionToBankService("Помощь детям", 
                        CharityServiceComboBox.GetItemText(CharityServiceComboBox.SelectedItem),
                        "Charity", sum, cardNumber, personalAccount, DataStorage.idClient
                        );
        ...
        {
            const string IsCorrectPersonalAccountNumberRegex = "^[0-9]{10}$";
            return Regex.IsMatch(personalAccount, IsCorrectPersonalAccountNumberRegex);
